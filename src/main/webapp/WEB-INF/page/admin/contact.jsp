            ...
            <div class="row">
                <div class="col-lg-12 col-md-12 col-sm-12 col-xs-12">
                    <div class="card">
                        ...
                        <form id="list-form" action="${ctx}/contact/admin" method="post">
                            <%--页码--%>
                                ...
                                <div class="menus">
                                    <div class="search-box input-group">
                                        ...
                                        <div class="form-line datetime-input">
                                            <input type="date" name="beginDay" value="<fmt:formatDate pattern="yyyy-MM-dd" value="${result.beginDay}"/>" class="form-control" placeholder="开始日期">
                                        </div>
                                        <div class="form-line datetime-input">
                                            <input type="date" name="endDay" value="<fmt:formatDate pattern="yyyy-MM-dd" value="${result.endDay}"/>" class="form-control" placeholder="结束日期">
                                        </div>
                                        <span class="input-group-addon">
                                        <i class="material-icons">search</i>
                                    </span>
                                        <div class="form-line keyword">
                                            <input type="text" name="keyword" value="${result.keyword}" class="form-control" placeholder="主题、内容等">
                                        </div>
                                    ...
                                    <div class="page-box">
                                        ...
                                        <nav>
                                            <ul class="pagination">
                                                <li id="prevPage" >
                                                    <!--上一页-->
                                                    <a onclick="go(${result.pageNo - 1})"><i class="material-icons">chevron_left</i></a>
                                                </li>

                                                <c:forEach begin="1" end="${result.totalPages}" var="index">
                                                    <c:choose>
                                                        <c:when test="${index == result.pageNo}">
                                                            <li class="active"><a>${result.pageNo}</a></li>
                                                        </c:when>
                                                        <c:otherwise>
                                                            <li><a onclick="go(${index})" class="waves-effect">${index}</a></li>
                                                        </c:otherwise>
                                                    </c:choose>
                                                </c:forEach>
                                                <li id="nextPage">
                                                    <!--下一页-->
                                                    <a onclick="go(${result.pageNo + 1})" class="waves-effect"><i class="material-icons">chevron_right</i></a>
                                                </li>
                                            </ul>
                                        </nav>
                                    </div>
    ...
    <script>
        ...
        function view(contact) {
            ...
            for (const k in contact){
                $viewForm.find('[name='+ k +']').val(contact[k])
            }
            //发送异步请求告诉服务器这条记录已查看
            $.getJSON('${ctx}/contact/read', {id:contact.id}, function(data){
                if (data.success){
                    //利用JS让对应checkbox打钩
                    $('#read-'+contact.id).attr('checked',true)
                }else {
                    console.log("查看失败")
                }
            })
